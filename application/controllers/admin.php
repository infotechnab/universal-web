...
class admin extends CI_Controller {
   ...
   public function index() {       
           ...
           if (($this->form_validation->run() == TRUE))
            {
             if ($_FILES && $_FILES['file']['name'] !== "")
             {
                ...
                $body = $this->input->post('area1');
                $status = $this->input->post('status');
                $this->dbmodel->update_page($id, $title, $body, $image, $status);
     ...
     function album()
    {
       ...
       $this->load->view('admin/templates/header');
       $this->load->view('admin/templates/menu');
       $this->load->view('admin/album/index');
    ...
    function add_album()
    {
        if ($this->session->userdata('logged_in')) {
            ...
            if (($this->form_validation->run() == FALSE)) {
                ...
                $this->load->view('admin/album/index', $error);
            } else {
    ...
    {    
        ...
        $this->load->view('admin/templates/header');
        $this->load->view('admin/templates/menu');
        $this->load->view('admin/album/gallery',$data);
        $this->load->view('admin/templates/footer');
    }
  ...
  //  ===================== slider ===========================================
    ...
    function addslider()
    {
         if ($this->session->userdata('logged_in')) {
            ...
            $config['allowed_types'] = 'gif|jpg|png';
            $config['max_size'] = '500';
            $config['max_width'] = '952';
            $config['max_height'] = '402';
    ...
    public function updateslider() {
        if ($this->session->userdata('logged_in')) {
            ...
            if (($this->form_validation->run() == FALSE)||(!$this->upload->do_upload())) {
                ...
                $this->dbmodel->update_slider($sid, $title, $image);
